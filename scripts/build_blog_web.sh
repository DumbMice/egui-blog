...
while test $# -gt 0; do
  case "$1" in
    -h|--help)
      echo "build_blog_web.sh [--release] [--glow] [--open]"
      echo ""
...
FINAL_WASM_PATH=web_blog/${OUT_FILE_NAME}_bg.wasm
...
wasm-bindgen "${WASM_PATH}" --out-dir web_blog --out-name ${OUT_FILE_NAME} --no-modules --no-typescript
