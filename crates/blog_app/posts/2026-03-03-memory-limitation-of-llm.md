...
The $O(T^2)$ scaling of computation with sequence length $T$ poses a fundamental challenge for deploying LLMs as general-purpose agents requiring long-term context.
...
Texts are _tokenized_ as a sequence of tokens $(bold(x)_1, dots, bold(x)_T)$, and each token $bold(x)_t in RR^(d_"tok")$ is a vector containing $d_"tok"$ _feature_ components.
For example, in GPT-3 (175B parameter model), $d_"tok"=12288$.

While focusing on different aspects of tokens, we might use different notations:

- $x$ indicates a sequence of tokens $(bold(x)_1, dots, bold(x)_T)$.
- bold $bold(x)_t$ indicates a token at position $t$.
- tensor notation $lr(x^(mu))_t$ indicates the component with index $1<=mu<=d_"tok"$ of a token at position $t$.
...
When $bold(x)_i$ attends to $bold(x)_j$, this establishes a __directional__ relationship.
An example of directional or asymmetric relationship:

$ "Bob" attach(limits(->), b:"love") "Alice" arrow.r.double.not "Alice" attach(limits(->), b:"love") "Bob" $.

Since symmetric operations like dot products cannot capture directionality, self-attention employs __a learnable asymmetric form__ to represent these directed connections.

Self-attention computes two projections for each token $bold(x)_t$:
...
where a sum over $mu$ is implied when it appears twice, one time as an upper index and the other time as a lower index.
This contraction of repeated Greek indices, often called _Einstein notation_ is used throughout this post.

__Euclidean Geometry Assumption__: In this post, we work in Euclidean space where the metric tensor is the identity matrix $delta_(alpha beta)$. Consequently, superscript and subscript indices can be interchanged freely: $a^(alpha) = a_(alpha)$ and $lr(K^(alpha))_(mu) = K_(alpha mu)$. This simplifies notation while maintaining mathematical rigor for our purposes.

With this notation established, we can now formalize the directional relationship between tokens. We define the __query score__, $r(x_i,x_j)$, as
...
Since $Q$ and $K$ are independent learnable matrices, $R$ is generally _asymmetric_, endowing $r(dot.op, dot.op)$ an __asymmetric bilinear form__,
...
_You can skip this part if you are familiar with tensor notations_
...
| $lr(x^(mu))_t$       | Token feature matrix         | $1<=mu<=d_"tok"$ : token, $1<=t<=T$ : position              |
| $lr(q^(alpha))_t$    | Query vector components      | $1<=alpha<=d$ : attention, $1<=t<=T$ : position             |
| $lr(k^(alpha))_t$    | Key vector components        | $1<=alpha<=d$ : attention, $1<=t<=T$ : position             |
| $lr(v^(alpha))_t$    | Value vector components      | $1<=alpha<=d$ : attention, $1<=t<=T$ : position             |
| $lr(Q^(alpha))_(mu)$ | Query projection matrix      | $1<=alpha<=d$ : attention, $1<=mu<=d_"tok"$ : token |
| $lr(K^(alpha))_(mu)$ | Key projection matrix        | $1<=alpha<=d$ : attention, $1<=mu<=d_"tok"$ : token |
| $lr(V^(alpha))_(mu)$ | Value projection matrix      | $1<=alpha<=d$ : attention, $1<=mu<=d_"tok"$ : token |

__Index Convention__: Greek letters index features: early Greek ($alpha, beta, gamma$) for attention feature dimension $d$, later Greek ($mu, nu, rho, sigma, tau$) for token feature dimension $d_"tok"$. Latin letters ($i, j, t, s$) index positions.

__Einstein Summation__: Repeated Greek indices (one upper, one lower) imply summation. Index placement can be swapped for summation: $a^(mu) b_(mu) = a_(mu) b^(mu)$. For dot products: $bold(q)_i dot.op bold(k)_j= lr(q^(alpha))_i lr(k_(alpha))_j$ sums over $alpha$.
...
The _query score_ $r(x_i, x_j)$ measures directional relationships between tokens. To use it for attention, we convert it into a probability distribution $bold(p)_i$ over tokens $x_j$. This allows us to interpret self-attention as computing the expectation of an _observable_ called _value_ $V$ over this distribution induced by query scores.
...
In neural networks, the corresponding notion is the softmax function:

$ "softmax"( bold(x))_i = exp(x_i)/(sum_i exp(x_i)) $

which maps an array of values $bold(x)equiv (x_i, ..., x_n)$ to a probability distribution $"softmax"(bold(x))$ with entries specified as  $("softmax"(bold(x))_i, ..., "softmax"(bold(x))_n)$.
...
$ mat(delim: #none, align: #left,  EE[x^(mu)] = 0",", "Var"[x^(mu)]=1 ; EE[lr(Q^(alpha))_(mu)] = 0",", "Var"[lr(Q^(alpha))_(mu)] = 1/d_"tok" ; EE[lr(K^(alpha))_(mu)] = 0",", "Var"[lr(K^(alpha))_(mu)] = 1/d_"tok".) $
...
To get rid of this dependency, we define a __normalized bilinear matrix__,

$ lr(hat(R)_(mu nu)) = lr(R_(mu nu)) / sqrt(d), $

and the __normalized query score__ $hat(r)_(i j)$ as
...
With this probability distribution, we can define the expected value of any _observable_ $O(dot.op)$ of $x_j$ under $bold(p)_i$ as
...
where $V$ is a third matrix of linear transformation $RR^(d_"tok") -> RR^d$. The value projection could be interpreted as a **value observable** over tokens.
...
The aggregated value from attended tokens equals the expectation of $V$:

$ EE_(x_j tilde bold(p)_i)[V] = sum_j (bold(p)_i)_j V bold(x_j) $

where $V$ denotes both the value observable (LHS) and its matrix representation (RHS).

__Key Insight__: Self-attention performs three operations:

1. __Score computation__: Calculate directional relationships ($Q,K$)
2. __Distribution formation__: Convert scores to probabilities (softmax)
3. __Value extraction__: Weighted sum of transformed tokens ($V$)

Thus, _self-attention computes the expectation of the value observable over a distribution induced by normalized query scores_.
...
$ lr(y^(alpha))_t = sum_(j=1)^T (exp(hat(r)_(t j)))/(Z_t) lr(V^(alpha))_(mu) lr(x^(mu))_j = (sum_(j=1)^T exp(lr(x^(mu))_t lr(hat(R)_(mu nu)) lr(x^(nu))_j) lr(V^(alpha))_(tau) lr(x^(tau))_j) /(sum_(i=1)^T exp(lr(x^(rho))_t lr(hat(R)_(rho sigma)) lr(x^(sigma))_i)). $
...
__Memory Implications__: Causal masking produces triangular attention patterns but preserves $O(T)$ complexity for each token. Each token attends to all predecessors, demanding quadratic memory for attention matrices as sequence length grows.
...
From the causal self-attention equation:

$ lr(y^(alpha))_t =  (sum_(j=1)^(t-1) "exp"(lr(x^(mu))_t lr(hat(R)_(mu nu)) lr(x^(nu))_j) lr(V^(alpha))_(tau) lr(x^(tau))_j )/(sum_(i=1)^(t-1) "exp"(lr(x^(rho))_t lr(hat(R)_(rho sigma)) lr(x^(sigma))_i)) $

we observe two critical scaling behaviors:

1. __Computation per token__: $O(t)$ operations for each $t$
2. __Total computation__: $O(T^2)$ for the full sequence
3. __Memory for attention scores__: $O(T^2)$ to store all $hat(r)_(i j)$
